...
class Mlp(nn.Module):
    def __init__(self, conv_layer, linear_layer, in_features, hidden_features=None, out_features=None,
        ...
        self.conv1 = conv_layer(in_features, hidden_features, 1, stride=1, padding=0, bias=False)
        self.act1 = act_layer()
        if self.spatial_conv:
            self.conv2 = conv_layer(hidden_features, hidden_features, 3, stride=1, padding=1,
                                    groups=self.group, bias=False)
            self.act2 = act_layer()
        self.conv3 = conv_layer(hidden_features, out_features, 1, stride=1, padding=0, bias=False)
...
class Attention(nn.Module):
    def __init__(self, conv_layer, linear_layer, dim, num_heads=8, head_dim_ratio=1., qkv_bias=False, qk_scale=None,
        ...
        self.qkv = conv_layer(dim, head_dim * num_heads * 3, 1, stride=1, padding=0, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = conv_layer(self.head_dim * self.num_heads, dim, 1, stride=1, padding=0, bias=False)
...
class Block(nn.Module):
    def __init__(self, conv_layer, linear_layer, dim, num_heads, head_dim_ratio=1., mlp_ratio=4., qkv_bias=False,
                 qk_scale=None,
        ...
        if not attn_disabled:
            ...
            self.attn = Attention(conv_layer, linear_layer, dim, num_heads=num_heads, head_dim_ratio=head_dim_ratio,
                                  qkv_bias=qkv_bias,
...
class Visformer(nn.Module):
    def __init__(self, conv_layer, linear_layer, img_size=224, patch_size=16, init_channels=32, num_classes=1000,
        ...
        if self.vit_embedding:
            ...
            self.patch_embed1 = PatchEmbed(conv_layer, linear_layer, img_size=img_size, patch_size=16, in_chans=3,
                                           embed_dim=embed_dim,
            ...
            if self.init_channels is None:
                self.using_stem = False
                self.patch_embed1 = PatchEmbed(conv_layer, linear_layer, img_size=img_size, patch_size=8, in_chans=3,
                ...
                self.stem = nn.Sequential(
                    conv_layer(3, self.init_channels, 7, stride=2, padding=3, bias=False),
                    BatchNorm(self.init_channels),
                    nn.ReLU(inplace=True)
                )
                img_size //= 2
                self.patch_embed1 = PatchEmbed(conv_layer, linear_layer, img_size=img_size, patch_size=4,
        ...
        if not self.vit_embedding:
            self.patch_embed2 = PatchEmbed(conv_layer, linear_layer, img_size=img_size, patch_size=2,
        ...
        # stage 3
        if not self.vit_embedding:
            self.patch_embed3 = PatchEmbed(conv_layer, linear_layer, img_size=img_size, patch_size=2,
                                           in_chans=embed_dim, embed_dim=embed_dim * 2,
                                           norm_layer=embedding_norm)
            img_size //= 2
            if self.pos_embed:
        ...
        if not self.vit_embedding:
            ...
            self.head = linear_layer(embed_dim * 2, num_classes)
        else:
            self.norm = norm_layer(embed_dim)
    ...
    def _init_weights(self, m):
        if isinstance(m, self.linear_layer):
            ...
            nn.init.constant_(m.bias, 0)
            nn.init.constant_(m.weight, 1.0)
        elif isinstance(m, self.conv_layer):
            if self.conv_init:
...
def visformer_tiny(conv_layer, linear_layer, input_size, num_classes, data_name, pretrained, **kwargs):
    model = Visformer(conv_layer, linear_layer, img_size=input_size, init_channels=16, embed_dim=192, depth=[7, 4, 4],
