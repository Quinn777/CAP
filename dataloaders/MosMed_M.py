...
class COVID_1110_Mediastinal_Folder(Dataset):
    def __init__(self, data_dir, partition, aug):
        self.base_dir = os.path.join(data_dir, "MosMed_M")
        ...
        if self.partition == "train":
            ...
            self.post_transform = transforms.Compose(
                [
                    # autoaugment.RandAugment(num_ops=1, magnitude=1),
                    transforms.ToTensor()
                ])
...
class MosMed_M:
    def __init__(self, data_dir, batch_size, *args):
        ...
        self.train_data = COVID_1110_Mediastinal_Folder(data_dir, "train", args[0])
        self.val_data = COVID_1110_Mediastinal_Folder(data_dir, "val", args[0])
